...
class ForestP2 extends Scene
{
    ...
    override public function onPropInteract(prop : Prop) : Void
    {
        if (Std.is(prop, LianaPlaced))
        {
            if (!plankPlaced)
            {
                Dialog.show(this, sherlock, Lang.get("scene.forestp2.cant_reach"), "default", "bottom").sound("sherlock.sound.dialog");
            }
            else
            {
                Dialog.show(this, sherlock, Lang.get("scene.forestp2.bare_hands"), "default", "bottom").sound("sherlock.sound.taunt");
            }
            return;
        }
        ...
        {
            Dialog.show(this, sherlock, Lang.get("scene.forestp2.slippery_rock")).sound("sherlock.sound.wonder");
            return;
        }

        if (Std.is(prop, BoulderWithPlank))
        {
            Dialog.show(this, sherlock, Lang.get("scene.forestp2.unstable_support")).sound("sherlock.sound.dialog");
        }
    }
    ...
    override public function onItemUse(prop : Prop, item : Item) : Void
    {
        if (Std.is(prop, LianaPlaced) && Std.is(item, Knife))
        {
            ...
            Dialog.show(this, sherlock, Lang.get("scene.forestp2.vine_rope")).sound("sherlock.sound.surprise");
        ...
        {
            Dialog.show(this, sherlock, Lang.get("scene.forestp2.dull_knife")).sound("sherlock.sound.dialog");
            return;
        }

        if (Std.is(prop, LianaPlaced) && Std.is(item, Scissors))
        {
            Dialog.show(this, sherlock, Lang.get("scene.forestp2.vine_scissors")).sound("sherlock.sound.dialog");
            return;
        }
        ...
        if (Std.is(prop, Boulder) && Std.is(item, Plank))
        {
            ...
            Dialog.show(this, sherlock, Lang.get("scene.forestp2.good_idea_vine")).sound("sherlock.sound.surprise");
            return;
        }

        Dialog.show(this, sherlock, Lang.get("scene.forestp2.what_doing")).sound("sherlock.sound.taunt");
    }

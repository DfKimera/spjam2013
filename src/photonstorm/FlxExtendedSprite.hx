...
class FlxExtendedSprite extends FlxSprite {
	 ...
	 */
    public function new(X:Float = 0, Y:Float = 0, SimpleGraphic:FlxGraphicAsset = null) {
    ...
    function updateGravity() {
        ...
        if (velocity.x != 0) {
            if (acceleration.x < 0) {
                ...
                if (touching.has(FlxDirectionFlags.WALL)) {
                    drag.y = frictionY;

                    if (!wasTouching.has(FlxDirectionFlags.WALL)) {
        ...
        if (velocity.y != 0) {
            if (acceleration.y < 0) {
                ...
                if (touching.has(FlxDirectionFlags.CEILING)) {
                    drag.x = frictionX;

                    if (!wasTouching.has(FlxDirectionFlags.CEILING)) {
                ...
                if (touching.has(FlxDirectionFlags.FLOOR)) {
                    ...
                    if (!wasTouching.has(FlxDirectionFlags.FLOOR)) {
                        if (velocity.y > -toleranceY) {
                            //trace("(down) velocity.y", velocity.y, "stopped via tolerance break", toleranceY);
                            velocity.y = 0;
                        }
    ...
    function checkForClick() {
        if (mouseOver && FlxG.mouse.justPressed) {
            ...
            if (clickPixelPerfect && mouseOver) {
                FlxMouseControl.addToStack(this);
                return;
            }

            if (dragPixelPerfect && mouseOver) {
                FlxMouseControl.addToStack(this);
                return;
            }
        }

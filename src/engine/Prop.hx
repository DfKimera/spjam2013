...
class Prop extends FlxExtendedSprite {
	...
	public function debug():String {
		return Type.getClassName(Type.getClass(this)) + " @ " + this.x + "," + this.y;
	}
	...
	private function _onItemUse(item:Item):Void {
		trace("Used item on prop", this.debug(), item);
	...
	public static function placeOnScene(scene:Scene, prop:Prop, x:Int, y:Int):Prop {
		...
		trace("Prop placed: ", prop.debug());
		return prop;
	}

	public static function bringToFront(prop:Prop, offsetFromTop:Int): Void {
		prop.ID = 0x7FFFFFFF - 100;
	}
}
